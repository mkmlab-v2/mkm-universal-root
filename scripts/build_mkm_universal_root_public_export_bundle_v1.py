...
ISSUE_TEMPLATES_SSOT = ROOT / "docs/final/artifacts/mkm_universal_root_github_issue_templates_v1"
...
def materialize(manifest: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    ...
    if ISSUE_TEMPLATES_SSOT.is_dir():
        tpl_dst = out_dir / ".github/ISSUE_TEMPLATE"
        tpl_dst.mkdir(parents=True, exist_ok=True)
        for src in sorted(ISSUE_TEMPLATES_SSOT.iterdir()):
            if not src.is_file():
                continue
            shutil.copy2(src, tpl_dst / src.name)
            copied.append(f".github/ISSUE_TEMPLATE/{src.name} (from issue-template SSOT)")
